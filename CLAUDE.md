...
`calculateCompetitionRanks(statsDict, key, idKey)` and `standingsRankingFn` (module scope in `server.js`, just above `/api/standings`) are the only ranking implementation: wins desc, losses asc, ties asc, competition ranking (identical W-L-T triples share a rank, next rank skipped). `GET /api/standings` uses them for the table; `GET /api/standings/history?year=` re-ranks every cumulative week snapshot with the same helper and returns `{ year, weeks, players: [{ id (firebaseUid), name, ranks[], rec, perfect[] }], playerCount }` for the **Standings Race** page (`/standings/race`, `src/pages/StandingsRace.jsx` + `src/components/RankRaceChart.jsx`, geometry in `src/utils/rankRace.js` — hand-rolled SVG, no chart library). `perfect` uses `isThreeZeroWeek()` — the same prize-eligible 3-0 predicate `/api/three-zero-standings` counts — so green markers on the chart equal a player's 3-0 Weeks total. Users without a `firebaseUid` are omitted from history (they are `rank '-'` in the table); members with no picks are ranked (tied at the bottom) in both. On the chart the viewer's own line is always drawn in gray-900 ink with a "· You" label tag (deliberately overriding its category color, and dimming only to 35% when another line is focused), and the focused line (pinned or hovered) gets a white halo, a dot per week, and an ink stroke when it would otherwise be gray — the page opens pinned to the viewer.
