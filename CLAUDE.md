...
Route gating lives in `src/App.jsx`: `<PrivateRoute>` (login required), `<PrivateRoute adminOnly>` (admin role), `<SeasonAccessGuard>` (blocks non-admins without an explicit `seasons.<key>.active === true` entry for the active season behind a full-page blocked screen with a Sign Out button; fails open when the active season is unset/unfetchable or the Mongo profile didn't load — `activeSeason` is fetched once in `AuthContext` during auth processing), and `<ProfileSetupGuard>` which forces users to `/setup-profile` until `venmoHandle` and `cellPhone` are set. Nesting order: `PrivateRoute` → `SeasonAccessGuard` → `ProfileSetupGuard` (so a blocked user sees the blocked screen, not profile setup; `/setup-profile` itself is also wrapped). Routes: `/`, `/locks`, `/weekly`, `/standings`, `/standings/race`, `/awards`, `/snydermetrics`, `/settings`, `/admin`.

### Authorization is almost entirely client-side — know this before touching routes
`backend/server.js` defines `authenticateUser()` (verifies a Firebase ID token from `Authorization: Bearer …`), but it is applied to exactly **two** routes: `GET /api/picks/check-completion` and `GET /api/picks/secure-user-picks`. Everything else — user create/read/update, whitelist management, pick submission, active-year switching, payout settings, announcements, prize pool, awards publish/unpublish, manual awards — is unauthenticated. Correspondingly, `Standings.jsx` is the only frontend file that attaches a bearer token.
...
**Users carry a per-season membership/dues map**: `user.seasons` is keyed by `String(seasonKey)` (e.g. `{"2025": {active, duesPaid, dateDuesPaid}, "2026_preseason": {…}}`). The old top-level `duesPaid`/`dateDuesPaid` fields were migrated into `seasons.2025` and are stripped by `PUT /api/users/:id`, which also refuses whole-map `seasons` writes — per-season edits go through a `seasonUpdate: {season, active, duesPaid, dateDuesPaid}` body field translated server-side into dot-notation `$set`s. `POST /api/users` seeds new users with the active season (`{active: true, duesPaid: false}`). **Membership gates a season's views by explicit opt-out only**: `seasonMembersQuery(seasonKey)` in `server.js` (mirrored by `isSeasonMember()` in `src/utils/seasonFormatter.js`) excludes a user from that season's standings, 3-0 standings, awards, awards summary, manual-award pick list, and the Weekly Locks page only when `seasons.<key>.active === false`; a **missing entry counts as a member** (2024 has no entries at all, and pre-migration users have none), so history never shifts. Their picks stay in the DB and reappear if Active is re-checked. The admin User Management table also hides users who are inactive for the selected season by default — a "Show Inactive Users" checkbox reveals them (grayed) for re-activation; "Not set" users always show since they count as members in standings. It shows "Not set" for users without an entry and only sends `seasonUpdate` when the membership/dues fields actually changed. **App access and pick submission use a deliberately separate, stricter predicate**: `isActiveSeasonParticipant()` in `seasonFormatter.js` (frontend, used by `<SeasonAccessGuard>`) and an inline gate in `POST /api/picks` both require an explicit `active: true` — so Inactive AND "Not set" users are locked out of the app for the active season and get a 403 on pick submission (admins exempt in both), while remaining counted in standings/history. Don't merge the two predicates. The picks gate keys on the submitted `year` (also 400s on an invalid season key) and its user lookup is reused by the Apps Script block below. **There is deliberately no way to delete a user** (the `DELETE /api/users/:id` route and the admin trash button were removed 2026-08-28): the user doc is global, every season's aggregation joins picks to it at read time, and deleting one silently dropped their picks from every past season (ranks, payouts, 3-0 shares, and award winners all changed). The admin table's "Set Inactive"/"Set Active" button toggles membership for the selected season instead. `DELETE /api/whitelist/:email` still exists for un-registered pre-approved emails. The migration script backfilled membership from picks history (deliberately skipping season 2024) and is safe to re-run — it never overwrites existing season entries.
...
### Standings ranking is shared code — keep the table and the race chart in step
`calculateCompetitionRanks(statsDict, key, idKey)` and `standingsRankingFn` (module scope in `server.js`, just above `/api/standings`) are the only ranking implementation: wins desc, losses asc, ties asc, competition ranking (identical W-L-T triples share a rank, next rank skipped). `GET /api/standings` uses them for the table; `GET /api/standings/history?year=` re-ranks every cumulative week snapshot with the same helper and returns `{ year, weeks, players: [{ id (firebaseUid), name, ranks[], rec, recs[], weekRecs[], perfect[] }], playerCount }` for the **Standings Race** page (`/standings/race`, `src/pages/StandingsRace.jsx` + `src/components/RankRaceChart.jsx`, geometry in `src/utils/rankRace.js` — hand-rolled SVG, no chart library). `perfect` uses `isThreeZeroWeek()` — the same prize-eligible 3-0 predicate `/api/three-zero-standings` counts — so green markers on the chart equal a player's 3-0 Weeks total. `recs[w]` (cumulative "W-L-T" through week w — the exact triple ranked for `ranks[w]`, so it must stay in step with `/api/standings` totals) and `weekRecs[w]` (that week alone) are index-aligned with `ranks` and feed the tooltip's "Week N record" / "Record thru Wk N" rows; the chart omits a row when an older payload lacks the array, and the endpoint deliberately sends no per-week submitted-pick count (it would reveal who has picked in the live week). Users without a `firebaseUid` are omitted from history (they are `rank '-'` in the table); members with no picks are ranked (tied at the bottom) in both. On the chart the viewer's own line is always drawn in gray-900 ink with a "· You" label tag (deliberately overriding its category color, and dimming only to 35% when another line is focused), and the focused line (pinned or hovered) gets a white halo, a dot per week, and an ink stroke when it would otherwise be gray — the page opens pinned to the viewer. The **Players** popover (`src/components/RacePlayerPicker.jsx`) shows/hides players: `hiddenIds` is plain page state (deliberately not persisted — every visit opens with everyone), and hiding is render-only via `applyVisibility()` in `rankRace.js` — `buildRaceModel` never sees visibility, so the axis, ticks, prize cutoff, tie fan-out, categories, colors and every rank number stay league-wide and a kept player's path never changes. `plottablePlayers()` is the single roster source for the chart, picker and counts. Any picker change clears the pin; "Show all" re-pins the viewer. A filtered view of ≤ `SPARSE_MAX` (10) players is "sparse": gray lines get weight, end dots and — when there are ≤ 3 of them — one of the three `COMPARE_COLORS` (the only hues that validated as colorblind-distinct next to the top-5 blues and last-place red; don't extend the list without re-validating), and focusing a line fades the others to 45% instead of 8%.
