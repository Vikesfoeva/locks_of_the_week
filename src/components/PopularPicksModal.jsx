...
const PopularPicksModal = ({ picks, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full">
        <h2 className="text-2xl font-bold mb-4">Top 5 Popular Picks</h2>
        {popularPicks.length > 0 ? (
          <ul>
            {popularPicks.map((pick, index) => (
              <li key={index} className="mb-2 p-2 border-b">
                <p className="font-semibold">{pick.gameDisplay}</p>
                <p>{pick.pickDisplay} - <span className="font-bold">{pick.count} {pick.count === 1 ? 'pick' : 'picks'}</span></p>
              </li>
            ))}
          </ul>
        ) : (
          <p>Not enough data to determine popular picks.</p>
        )}
        <button
...
export default PopularPicksModal; 